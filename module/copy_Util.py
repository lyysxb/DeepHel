...
def getReverseSequence(sequence):
    base_map = {'A': 'T', 'T': 'A', 'C': 'G', 'G': 'C', 'a':'T', 't':'A','c':'G','g':'C'}
    res = ''
    length = len(sequence)
    i = length - 1
    while i >= 0:
...
def get_query_copies(cur_segments, query_contigs, subject_path, query_coverage, subject_coverage, query_fixed_extend_base_threshold=4000, subject_fixed_extend_base_threshold=4000, max_copy_num=100):
    all_copies = {}
...
def multiple_alignment_blast_and_get_copies_v1(repeats_path):
    ...
    if split_ref_dir.endswith(".fa"):
       #print("end with fa")
       align_command = 'blastn -db ' + split_ref_dir + ' -num_threads ' \
                            + str(1) + ' -query ' + split_repeats_path + ' -evalue 1e-5 -outfmt 6 > ' + blastn2Results_path
       os.system(align_command)
...
def flank_region_align_v5_2(candidate_sequence_path, real_TEs, flanking_len, reference, split_ref_dir,
                          TE_type, tmp_output_dir, threads, ref_index, log, subset_script_path, plant, debug,
                          iter_num, result_type='cons'):
    log.logger.info('------Determination of homology in regions outside the boundaries of ' + TE_type + ' copies')
    starttime = time.time()
